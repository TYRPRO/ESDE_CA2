...
module.exports.getFileData = (userId, pageNumber, search) => {
        ...
        SQLParameters = [userId,limit,offset,userId];
        } else {
            designFileDataQuery = `SELECT file_id,cloudinary_url,design_title,design_description 
            FROM file  WHERE created_by_id=? AND design_title LIKE concat('%',?,'%')  LIMIT ? OFFSET ?;
            ...
            SQLParameters = [userId,search,limit,offset,userId,search];
        }
        //--------------------------------------------------------------------
        ...
        return new Promise((resolve, reject) => {
            ...
            pool.getConnection((err, connection) => {
                if (err) {
                    ...
                    connection.query(designFileDataQuery, SQLParameters, (err, results) => {
                        if (err) {
