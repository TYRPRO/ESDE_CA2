  ...
  return new Promise((resolve, reject) => {
    ...
    pool.getConnection((err, connection) => {
      if (err) {
        ...
        connection.query(
          `UPDATE user SET role_id = ? WHERE user_id= ?`, [newRoleId,recordId],
          (err, rows) => {
            if (err) {
              reject(err);
            } else {
...
module.exports.getUserData = (pageNumber, search) => {
  ...
  if (search == "" || search == null) {
    ...
    SQLParameters = [limit,offset];
  } else {
    userDataQuery = `SELECT user_id, fullname, email, role_name 
        FROM user INNER JOIN role ON user.role_id = role.role_id AND fullname LIKE concat('%',?,'%') LIMIT ? OFFSET ?;
    SET @total_records =(SELECT count(user_id) FROM user WHERE fullname LIKE concat('%',?,'%') );SELECT @total_records total_records;`;
    SQLParameters = [search,limit,offset,search];
  }
        ...
        connection.query(
          ...
          (err, results) => {
            if (err) {
              ...
              console.log(
                "Accessing total number of rows : ",
                results[2][0].total_records
              );
              resolve(results);
            }
            connection.release();
...
module.exports.getOneUserData = function (recordId) {
  ...
  userDataQuery =
    `SELECT user_id, fullname, email, user.role_id, role_name 
  ...
  return new Promise((resolve, reject) => {
    ...
    pool.getConnection((err, connection) => {
      if (err) {
        ...
        connection.query(
          `UPDATE file SET design_title = ?, design_description= ? WHERE file_id= ?`, [title, description,recordId],
          (err, rows) => {
            if (err) {
              reject(err);
            } else {
...
module.exports.createOneEmailInvitation = (
  userData,
  recipientName,
  recipientEmail
) => {
